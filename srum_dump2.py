...
def ole_timestamp(binblob):
    ...
    try:
        ...
        dt = datetime(1899, 12, 30, 0, 0, 0) + timedelta(days=int(td),
                                                         seconds=86400 * float("0.{}".format(ts)))
    except:
...
def load_interfaces(reg_file):
    ...
    for eachinterface in int_keys.subkeys():
        ...
        for eachprofile in eachinterface.subkey("Profiles").subkeys():
            profileid = [x.value() for x in list(eachprofile.values())
            ...
            for eachvalue in metadata:
                if eachvalue.name() in ["Channel Hints", "Band Channel Hints"]:
                    ...
                    profile_lookup[str(profileid)] = name.decode(
                        encoding="latin1")
...
def load_srumid_lookups(database):
    ...
    column_lookup = dict([(x.name, index)
                         for index, x in enumerate(lookup_table.columns)])
    for rec_entry_num in range(lookup_table.number_of_records):
        bin_blob = smart_retrieve(
            lookup_table, rec_entry_num, column_lookup['IdBlob'])
        ...
        id_lookup[smart_retrieve(
            lookup_table, rec_entry_num, column_lookup['IdIndex'])] = bin_blob
    return id_lookup
...
def load_template_tables(template_workbook):
    ...
    for each_sheet in sheets:
        ...
        for eachcolumn in range(1, template_sheet.max_column+1):
            ...
            if field_name == None:
                break
            template_style = template_sheet.cell(
                row=4, column=eachcolumn).style
            template_format = template_sheet.cell(
                row=3, column=eachcolumn).value
            template_value = template_sheet.cell(
                row=4, column=eachcolumn).value
            if not template_value:
                template_value = field_name
            template_field[field_name] = (
                template_style, template_format, template_value)
...
def format_output(val, eachformat, eachstyle, xls_sheet):
    ...
    if val == None:
        ...
        inttype = struct.unpack(">H6B", codecs.decode(
            format(val, '016x'), 'hex'))[0]
    ...
    try:
        ...
        new_cell.value = re.sub(
            r'[\000-\010]|[\013-\014]|[\016-\037]|[\x00-\x1f\x7f-\x9f]|[\uffff]', "", val)
    return new_cell
    ...
    for table_num in range(ese_db.number_of_tables):
        ...
        if ese_table.name in template_tables:
            ...
            for eachcol in ese_table.columns:
                if eachcol.name in tfields:
                    ...
                    header_row.append(WriteOnlyCell(
                        xls_sheet, value=eachcol.name))
        ...
        for row_num in range(ese_table.number_of_records):
            try:
                ...
                print("Skipping corrupt row {0} in the {1} table. Because {2}".format(
                    row_num, ese_table.name, str(e)))
                continue
            if ese_row == None:
                continue

            if not options.quiet and row_num % 500 == 0:
                print("\r|{0:-<50}| {1:3.2f}%".format("X"*(50 * row_num //
                      ese_table.number_of_records), 100*row_num/ese_table.number_of_records), end="")
...
def show_live_system_warning():
    ...
    layout = [
        ...
        [sg.Text(
            r"Try: 'fget -extract c:\windows\system32\sru\srudb.dat <a destination path>'")],
        [sg.Button("Close"), sg.Button("Download FGET")]
    ]
    if ctypes.windll.shell32.IsUserAnAdmin() == 1:
        layout[-1].append(sg.Button("Auto Extract"))
    pop_window = sg.Window(
        "WARNING", layout, no_titlebar=True, keep_on_top=True, border_depth=5)
    return_value = None
    while True:
        ...
        if event == "Download FGET":
            webbrowser.open(
                "https://github.com/MarkBaggett/srum-dump/blob/master/FGET.exe")
    ...
    try:
        ...
        if esentutl_path.exists():
            ...
            cmdline = r"{} /y c:\\windows\\system32\\sru\\srudb.dat /vss /d {}".format(
                str(esentutl_path), str(extracted_srum))
            print(cmdline)
            ...
            cmdline = r"{} /y c:\\windows\\system32\\config\\SOFTWARE /vss /d {}".format(
                str(esentutl_path), str(registry_file))
            print(cmdline)
            phandle = subprocess.Popen(
                cmdline, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            ...
            fget_binary = urllib.request.urlopen(
                'https://github.com/MarkBaggett/srum-dump/raw/master/FGET.exe').read()
            fget_file.write_bytes(fget_binary)
            print("Extracting srum with fget.exe")
            cmdline = r"{} -extract c:\\windows\\system32\\sru\srudb.dat {}".format(
                str(fget_file), str(extracted_srum))
            print(cmdline)
            phandle = subprocess.Popen(
                cmdline, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            out1, _ = phandle.communicate()
            cmdline = r"{} -extract c:\\windows\\system32\\config\SOFTWARE {}".format(
                str(fget_file), str(registry_file))
            print(cmdline)
            phandle = subprocess.Popen(
                cmdline, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            out2, _ = phandle.communicate()
